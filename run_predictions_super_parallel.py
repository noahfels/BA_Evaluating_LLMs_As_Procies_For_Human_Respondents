...
# Konfiguration
# ----------------------------
DATA_PATH = Path("./data/ess11_de_filtered.csv")
META_PATH = Path("./data/ess11_de_variables.csv")
OUT_WIDE_CSV = Path("./data/ess11_llm_predictions_wide.csv")
...
# Optionales Token-Throttling (bei 429 error durch TPM siehst)
TOKENS_PER_REQ_EST = 1650  # Grobe Schätzung
...
async def run_all_async(df: pd.DataFrame, meta: pd.DataFrame) -> None:
    ...
    # --------- NEU: mehrere Personen parallel (Chunking) ----------
    ...
    for i, row in enumerate(df.itertuples(index=False)):
        ...
        # Wenn genug Personen : warten + flushen
